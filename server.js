    ...
    while (queue.length > 0) {
        ...
        if (depth === times_count) {
            if (answer < revenue) {
                ...
                if (before > 0) console.log('   ' + answer + '원     -->     ' + (answer - before) + '원 증가!');
        ...
        for (let i = 0; i < len; i++) {
            ...
            /* 거리를 계산하자 */
            let move = getDistance(latitude, longitude, next_latitude, next_longitude);
            ...
            queue.push([depth + 1, next_latitude, next_longitude, next_revenue, next_visit, total_move + move]);
        }
    }
    console.log();
    console.log('------------- 추천 결과 -------------');
...
app.post('/store/info', getOwnerIdByEmail, async (req, res, next) => {
    next();
});
...
async function getJobIdByStoreId(req, res, next) {
    ...
    try {
        ...
        try {
            ...
            if (req.body['types'].length === 0) throw new Error('error: getTypeByJobId');
        } catch (exception) {
...
app.post('/owner/employment', getOwnerIdByEmail, async (req, res, next) => {
    next();
});
...
app.use('/owner/employment', async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < hour; i++) {
            if (start_hour.toString().length === 1) all_hours.push('0' + (start_hour + i).toString() + ':00:00');
...
function masage_data(latitude, longitude, data) {
    ...
    for (let i = 0; i < len; i++) {
        ...
        if (
            !databox[check[d['name']]]['work_date_and_type_and_id'][[d['work_date'], d['type'], d['order_id']]][
                'start_time_and_id'
            ].hasOwnProperty([d['start_time'], d['hourlyorders_id']])
        ) {
            databox[check[d['name']]]['work_date_and_type_and_id'][[d['work_date'], d['type'], d['order_id']]]['start_time_and_id'].push([
                d['start_time'],
                d['hourlyorders_id'],
            ]);
        }
...
function recur(idx, start_times, latitude, longitude, dict, hourly_orders, sum) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        tmp -= getDistance(latitude, longitude, dest_latitude, dest_longitude) * 2.5;
